#!/usr/bin/env python3
"""Generate the deterministic, non-sensitive M1D PCAP fixture.
...
def tls_handshake() -> tuple[bytes, bytes]:
    server_name = b"tls.example.test"
    name_list = struct.pack("!HBH", len(server_name) + 3, 0, len(server_name)) + server_name
...
def main() -> None:
    ...
    print(f"wrote {len(PACKETS)} packets, {len(payload)} bytes, sha256={digest}")
