...
if not hasattr(ingestion_core, "sha256_input_file") or not hasattr(
    ...
    raise ImportError(
        ...
        "bash -c 'source .venv/bin/activate && maturin develop --manifest-path ingestion/Cargo.toml' "
...
class M1DPipelineTests(unittest.TestCase):
    ...
    def test_legacy_output_matches_frozen_baseline_and_last_row_wins(self) -> None:
        ...
        # Git may check the golden fixture out with CRLF on Windows. Normalize
        # the fixture first, then compare with the native newline behavior of
        # the legacy text-mode writer.
        expected = (
            LEGACY_GOLDEN.read_bytes()
            .replace(b"\r\n", b"\n")
            .replace(b"\n", os.linesep.encode())
        )
    ...
    def test_canonical_only_flags_and_unverifiable_skip_ja4_are_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(SystemExit, "cannot verify the qualified JA4 runtime"):
            pipeline.run_pipeline(
                None,
                str(LOG_FIXTURE),
    ...
    def test_canonical_ja4_mode_is_forwarded_to_the_fresh_qualified_runtime(self) -> None:
        pcap = self.root / "qualified-ja4.pcap"
        pcap.write_bytes(b"qualified JA4 input")
        kept = self.root / "qualified-ja4-logs"
        seen: dict[str, bool] = {}
        ...
        def qualified_run(
            ...
            seen["use_ja4"] = use_ja4
            seen["canonical_mode"] = canonical_mode
            (Path(out_dir) / "conn.log").write_bytes(
                (LOG_FIXTURE / "conn.log").read_bytes()
            )
            (Path(out_dir) / "ssl.log").write_bytes(
                (LOG_FIXTURE / "ssl.log").read_bytes()
            )

        with mock.patch.object(pipeline, "run_zeek", side_effect=qualified_run):
            pipeline.run_pipeline(
                str(pcap),
                str(kept),
                str(self.root / "qualified-ja4-features.jsonl"),
                use_ja4=True,
                canonical_output_path=str(self.root / "qualified-ja4-canonical.jsonl"),
                sensor_id="sensor",
                observed_at=FIXED_OBSERVED_AT,
                feature_profile=pipeline.DETECTOR_FEATURE_PROFILE,
            )

        self.assertEqual(seen, {"use_ja4": True, "canonical_mode": True})
    ...
    def test_ja4_mode_retains_pcap_mutation_prepublication_guard(self) -> None:
        pcap = self.root / "mutable-ja4.pcap"
        pcap.write_bytes(b"original qualified JA4 evidence")
        features = self.root / "mutation-ja4-features.jsonl"
        canonical = self.root / "mutation-ja4-canonical.jsonl"
        kept = self.root / "mutation-ja4-kept"
        seen: dict[str, bool] = {}

        def mutate_after_qualified_zeek(
            pcap_path: str,
            out_dir: str,
            use_ja4: bool = False,
            canonical_mode: bool = False,
            ...
            Path(pcap_path).write_bytes(b"mutated during qualified JA4 replay")

        with mock.patch.object(
            pipeline, "run_zeek", side_effect=mutate_after_qualified_zeek
        ), self.assertRaisesRegex(SystemExit, "changed while Zeek"):
            pipeline.run_pipeline(
                str(pcap),
                str(kept),
                str(features),
