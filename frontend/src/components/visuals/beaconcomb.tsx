...
export function BeaconComb({ periods = 42, jitterPct = 4 }: { periods: number; jitterPct: number }) {
  const width = 480;
  ...
  return (
    <div>
      <Row text="This flow" ticks={beaconTicks} color={T.sevCrit} />
      ...
      <div style={{ marginTop: 12, fontFamily: "monospace", fontSize: 13, color: T.text }}>
        measured jitter <span style={{ color: T.sevCrit }}>{jitterPct}%</span>
      </div>
    </div>
  );
}